...
::: notes
Old failure: invented PubMed ID. Usual failure now: a real paper with the wrong claim; a rule set at the start can vanish mid-run; citation chips that look like proof. Citations and tool use do not eliminate checking. Artsi: interpretive error even with real citations.
:::
...
::: notes
The goal is a taxonomy they can keep when the posters change. Autocomplete finishes the next word. Chat is a conversation. Tools look things up or run a check. Product names change quickly; these three jobs are what to remember. Full chronology is in backup if someone wants GPT-2 through Fable.
:::
...
## These models try an answer, check it, and try again. {.viz}
...
::: notes
Vendors sell extra tokens before the answer — a try, test, revise loop you usually do not see. Left is the 2023 chatbot: first fluent sentence, ship it. Right is o1-class: hidden scratch work, then one answer. The toy is a 10 mg/kg dose that needs a 400 mg maximum: 50 kg × 10 = 500, which fails the maximum, so the revised answer is 400 mg. The line under the boxes is that in English. Coding tests had a unit-test check like that first; keep that history off the canvas unless someone asks. Do not turn this into Lab 2. Files and tools are the next slide. o1 Sep 2024; DeepSeek-R1 Jan 2025 if someone asks whether it was OpenAI-only.
:::
...
## Labs 1 through 4 run in two live blocks. {.viz}
...
::: notes
These three checks before they copy. Then thank them and take questions. Do not invent an AAP clinical AI guideline.
:::
...
::: notes
Stop here for the live hour. Thank them. Take questions. Do not invent an AAP clinical AI guideline. Packet is briefing/; presenter-kit is slides/presenter-kit.md. Appendix is take-home Labs 5–8, then backup. Linear deck: you can leave the rest unshown.
:::
